...
class ParseFile {
  ...
  parseLine(string, i) {
    ...
    if (this.validateLine(textArr, i)) {
      let obj = formatUtil.textToJSON(textArr, this.getIdx);
      obj = formatUtil.checkMiddleName(obj);
      fileUtil.writeToStream(this.writeStream,formatUtil.dataToString(obj) + '\n');
    }
  }
  ...
  closeStream() {
    ...
    fileUtil.deleteFile(this.options.inputDirectory, this.fileName);
    this.handleFileDone();
  }
